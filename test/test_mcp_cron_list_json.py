...
class TestBoundsAndSafety:
    ...
    def test_it_stays_under_the_response_ceiling_at_the_cap(self) -> None:
    ...
    def test_a_long_prompt_is_bounded(self) -> None:
        out = json.loads(_call([_job(message="x" * 5000)]))
        assert len(out["jobs"][0]["message"]) <= mcp_cron._JSON_MESSAGE_LEN
