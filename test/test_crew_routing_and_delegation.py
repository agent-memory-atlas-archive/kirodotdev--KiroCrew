...
def _materialize_private_stores(config):
    ...
    from kiro_crew.memory_stores import MEMBER_MEMORY_MANIFEST, memory_store_dir_for
    ...
    for name, record in config.memory_stores.items():
        ...
        (root / MEMBER_MEMORY_MANIFEST).write_text(
            json.dumps({"owner_member": record.owner_member, "memory_version": 2}),
            encoding="utf-8",
        )
...
class TestCrewToCrewDelegation:
    ...
    def test_a_declared_template_travels_and_an_undeclared_one_stays_empty(
        ...
        (tmp_path / "config.json").write_text(
            json.dumps(
                {
                    "memory_stores": {
                        "coding": {"owner_member": "declared", "memory_version": 2},
                        "email": {"owner_member": "undeclared", "memory_version": 2},
                    },
                    "default_agent": "kirocrew",
                    "agents": {
                        "kirocrew": {},
                        ...
                        "undeclared": {"memory_store": "email", "triggers": "draft"},
                    },
                }
            )
        )
        ...
        try:
            ...
            assert orch._resolve_delegate("undeclared") == ("", "email")
        finally:
            loader_mod._invalidate_config_cache()
    ...
    def test_an_invented_crew_refuses_delegation(self, crew_config):
        """Explicit unknown identity cannot quietly become an ordinary spawn."""
        with pytest.raises(ValueError, match="unknown Crew Member"):
            self._orchestrator()._resolve_delegate("ghost crew")
        assert self._orchestrator()._resolve_delegate("") == ("", "")

    def test_a_broken_config_offers_no_roster_and_refuses_named_delegation(
        self, tmp_path, monkeypatch
    ):
        monkeypatch.setenv("KIROCREW_HOME", str(tmp_path))
        ...
        try:
            ...
            with pytest.raises(ValueError, match="unknown Crew Member"):
                orch._resolve_delegate("coding crew")
        finally:
            loader_mod._invalidate_config_cache()
