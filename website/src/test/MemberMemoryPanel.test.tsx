...
const { api } = vi.hoisted(() => ({ api: {
  ...
  vectorSemanticWrite: vi.fn(), vectorSemanticDelete: vi.fn(), vectorEpisodicDelete: vi.fn(), correctMemberEpisode: vi.fn(),
...
beforeEach(() => {
  ...
  api.correctMemberEpisode.mockResolvedValue({ ok: true, changed: true })
...
async function chooseWriter() {
  ...
  fireEvent.click(await screen.findByRole('option', { name: 'writer · V2' }))
}
...
describe('private member memory lifecycle', () => {
  it('labels a named legacy store as usable Memory V1 without presenting private V2 actions', async () => {
    ...
    const guidance = screen.getByText(/Memory V1\. This member can keep working/)
    expect(guidance).toHaveTextContent('A new private Memory V2 starts empty')
    expect(guidance).toHaveTextContent('copy only what this member needs')
    expect(screen.queryByText('Private to this member')).toBeNull()
    expect(screen.queryByRole('button', { name: 'Copy knowledge' })).toBeNull()
  ...
  it.each(['absent', 'ownerless', 'unsupported-version'])('refuses a %s private identity instead of presenting the store id', async condition => {
    const catalog = condition === 'absent'
      ...
      : stores.map(s => s.name === MEMBER_STORE
        ? { ...s, ...(condition === 'ownerless' ? { owner_member: '' } : { memory_version: 3 }) }
        : s)
  ...
  it('keeps persisted pending restore visible after remount and blocks duplicate staging', async () => {
    ...
    for (let visit = 0; visit < 2; visit++) {
      fireEvent.mouseDown(screen.getByRole('tab', { name: 'Recovery' }), { button: 0, ctrlKey: false })
      expect(await screen.findByText('Restore is ready. Restart Kiro Crew (the gateway) to finish restoring this backup. The current memory store stays active until then.')).toBeInTheDocument()
  ...
  it('cancels a staged member restore with explicit success and keeps the original backup available', async () => {
    ...
    const status = await screen.findByText(/^Staged restore cancelled\. Current memory and the backup are unchanged\./)
    expect(status).toHaveTextContent('If memory recovery failed at startup, restart the gateway to try again.')
  ...
  it('uses each owner’s exact roster avatar in the header and switcher, including uploaded and pinned faces', async () => {
    ...
    const writer = await screen.findByRole('option', { name: 'writer · V2' })
    expect(writer.querySelector('img')).toHaveAttribute('src', writerSrc)
    expect(screen.getByRole('option', { name: 'Global Memory V1' }).querySelector('img')).toBeNull()
  ...
  it('shows the source member avatar while copying without replacing the target identity', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'Copy knowledge' }))
    const dialog = screen.getByRole('dialog', { name: /^Copy knowledge to/ })
    expect(dialog).toHaveAccessibleName('Copy knowledge to “reviewer”')
  ...
  it('copies only checked source items and keeps a skipped result visible', async () => {
    renderWithProviders(<MemoryTab refreshTrigger={0} />)
    await loaded()
    expect(api.memberMemoryPage.mock.calls.every(([store]) => store === MEMBER_STORE)).toBe(true)
    expect(screen.queryByText('The source memory remains unchanged; this action only adds selected knowledge to this member.')).toBeNull()
    expect(api.memorySeed).not.toHaveBeenCalled()
    fireEvent.click(screen.getByRole('button', { name: 'Copy knowledge' }))
    const dialog = screen.getByRole('dialog', { name: /^Copy knowledge to/ })
    await waitFor(() => expect(within(dialog).getByText('Copy selected items into this member. Existing items and the source stay unchanged; each copy records its origin.')).toBeVisible())
    ...
    expect(screen.getByRole('status')).toHaveTextContent('The source memory remains unchanged; this action only adds selected knowledge to this member.')
    fireEvent.click(screen.getAllByRole('button', { name: 'View details' })[0])
    expect(screen.getByText('Copied from Global Memory V1')).toBeInTheDocument()
  ...
  it.each(['member', 'copy'] as const)('preserves server-normalized text matches in the %s search', async surface => {
    ...
    const view = surface === 'copy' ? within(screen.getByRole('dialog', { name: /^Copy knowledge to/ })) : screen
  ...
  it('filters experiences without mixing in facts and returns to all memories', async () => {
    renderWithProviders(<MemoryTab refreshTrigger={0} />)
    await loaded()
    expect(screen.getByText('Facts are saved details, Rules guide how the member works, and Experiences are events the member can recall.', { exact: true })).toBeVisible()
