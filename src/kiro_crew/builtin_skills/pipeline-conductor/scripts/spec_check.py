...
``python3`` is whatever the operator has, which under a managed install is not
the interpreter Kiro Crew runs on — so the read gate below is frequently not
importable HERE. The script then re-runs itself under Kiro Crew's own
interpreter rather than refusing: the gate is relocated, never skipped.
...
try:
    ...
    # A skill's scripts are copied OUT of the package tree and run as bare files
    # under whichever ``python3`` the operator has, so ``kiro_crew`` is not
    # guaranteed to be importable. There is no safe degradation for a READ GATE:
    # falling back to ``read_text`` would reopen exactly the bypass the import
    # exists to close, and it would do so silently, in the case that is hardest
    # to notice. So this is not the fallback — ``main`` re-execs under an
    # interpreter that HAS the gate, and refuses only when there is none.
...
def bundled_python() -> str | None:
    ...
    Kiro Crew's own interpreter is the one carrying the package, and the
    ``kirocrew`` console script lives in that environment's script directory —
    so the interpreter is its SIBLING. That holds on every platform without
    special casing the directory name: POSIX puts both in ``bin/``, Windows puts
    both in ``Scripts/``. Only the executable's suffix differs.

    ``realpath`` matters: the launcher is frequently a symlink from a user's
    ``bin`` directory into the environment, and the sibling lookup has to run
    where the interpreter is, not next to the symlink.
    """
...
def _reexec_with_the_gate(spec: str) -> int:
    ...
    if interpreter is None:
        return _refuse_unenforceable("no interpreter was found beside the 'kirocrew' launcher")
    env = {**os.environ, _REEXEC_ENV: "1"}
    return subprocess.call([interpreter, os.path.abspath(__file__), "--spec", spec], env=env)
