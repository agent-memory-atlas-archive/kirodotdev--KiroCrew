      ...
      <div role="tablist" aria-label={i18nT('components.pullRequestPanel.pull_requests')} className="shrink-0 border-b border-border px-2 py-2 flex items-center gap-1 overflow-x-auto">
        {cappedSources.map(item => {
          ...
          >
            ...
            {/* The qualifier truncates in its own span so a deep GitLab
                subgroup path cannot grow a shrink-0 tab unboundedly; the full
                url stays reachable through the Btn's title. The reference
                (`MR !1`) never truncates — it is the part that must stay
                legible. */}
            {qualifier && <span className="truncate max-w-[18ch]">{qualifier}</span>}
