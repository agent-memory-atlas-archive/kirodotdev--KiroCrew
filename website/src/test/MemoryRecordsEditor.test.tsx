...
describe('memory record editing in both lineages', () => {
  it('explains selection actions before selection and restores the hint after clearing', async () => {
    ...
    const hint = 'Select memories to replace text or forget them. You will review a preview before saving.'
  ...
  it.each([
    ...
    if (privateMemory) {
      expect(screen.getByText(/the source memory stays unchanged/)).toBeVisible()
      expect(screen.queryByText('No memories yet')).toBeNull()
    } else {
      expect(screen.getByText('Facts, rules, and experiences saved in this memory will appear here.')).toBeVisible()
      expect(screen.queryByText('A fresh start for this member')).toBeNull()
      expect(screen.queryByText(/This private memory grows/)).toBeNull()
      expect(screen.queryByText(/the source memory stays unchanged/)).toBeNull()
    }
  ...
  it.each(['default', 'member-review'])('keeps explicit selection across server pages in %s and never writes before reviewed apply', async store => {
    ...
    expect(within(dialog).getByText(/If it expires, preview the changes again/)).toBeVisible()
  ...
  it.each(['default', 'member-review'])('keeps recovery limits visible before applying a scoped %s forget', async store => {
    ...
    const explanation = 'Remove these memories from future searches. There is no direct Undo here. To recover them, restore a backup that contains them; this returns the whole memory store to that snapshot after a gateway restart.'
...
it.each([
  ...
  { source: 'user_seed', label: 'Selected knowledge' },
...
it.each([
  ...
  { extra: { metadata: { source_ref: 'user_seed' } }, reference: 'Selected knowledge' },
...
it.each(['custom_or_registered', 'unknown', 'reference_identity'])('shows selected snippets and qualifies %s recall while keeping full context collapsed', async qualification => {
  ...
  const recall = await screen.findByRole('button', { name: 'Recall for this task' })
  await waitFor(() => expect(recall).toBeEnabled())
  const explanation = 'This is the context the member would receive for this task. Search results below are for browsing and editing.'
  ...
  const modelNotice = evidence.queryByText('Memory matching has not been validated with this model. Check the selected memories for this task.')
  ...
  expect(evidence.getByText('Rules are included. Open the details below to read them.')).toBeVisible()
...
it.each(['rules', 'empty'])('keeps the model notice visible in %s recall without factual matches', async contents => {
  ...
  fireEvent.click(recall)
  const evidence = within(await screen.findByTestId('memory-recall-evidence'))
  expect(evidence.getByText('Memory matching has not been validated with this model. Check the selected memories for this task.')).toBeVisible()
  if (contents === 'rules') {
