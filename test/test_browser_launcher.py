...
class TestOpenUrlArgv:
    ...
    def test_a_timed_out_open_is_still_recorded_for_shutdown(self):
        ...
        assert launcher._opened == {result.session}
        # A launch that FAILED outright started no daemon: nothing to reap.
        fake2, _calls2 = _runs([LIST_ABSENT, (1, "", "Error: boom")])
        with patch.object(launcher, "_run_cli", fake2):
            other = launcher.open_url(URL, "chat-2")
        assert other.session not in launcher._opened
...
class TestReveal:
    ...
    def test_sends_one_reveal_line_to_the_running_dashboard(self, request: pytest.FixtureRequest):
        ...
        thread.start()
        assert ready.wait(5)
        REAL_REVEAL("panel-0a1b2c-1234abcd", {launcher.SOCKETS_ENV: str(root)})
        thread.join(5)
    ...
    def test_a_missing_dashboard_is_silently_skipped(self, tmp_path: Path):
        ...
        REAL_REVEAL("panel-0a1b2c-1234abcd", {launcher.SOCKETS_ENV: str(tmp_path)})
...
def launch_mocks(tmp_path: Path):
    ...
    def fake_open(url: str, session_key: str):
        ...
        return launcher.LaunchResult(True, launcher.session_name(session_key), None)
