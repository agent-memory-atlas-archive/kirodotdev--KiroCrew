...
## Why diff-scoped, and when that ends

Diff mode exists ONLY to keep the landing PRs green: the sites are converted a
few at a time, and a whole-tree gate would charge every unconverted site to
whoever pushed next. It flips to whole-tree enforcement in the commit that lands
the last site — at which point ``MEMSTORE_BASE_REF`` stops being what makes the
gate bite. Until then the whole-tree count is printed as a NON-failing report so
the remaining backlog stays visible, and ``test/test_memory_store_seam.py`` pins
that count so it can only shrink deliberately.
