...
class FeishuDispatcher:
    ...
    def __init__(
        ...
        *,
        ...
        self._config_sub = live.subscribe(
            "feishu", "messaging", callback=self._on_config_change, name="FeishuDispatcher"
        )
    ...
    def _live_cfg(self) -> "KiroCrewConfig":
        ...
        try:
            ...
            logger.warning("Feishu: could not read live config; using the boot copy", exc_info=True)
            return self.cfg
    ...
    def _dm_scope(self, route: tuple[str, str], gen: int) -> str:
        ...
        re-keys a running conversation into a different session. Pinned per
        ROUTE, like every other piece of state here, so a group's scope cannot be
        decided by a turn in the sender's DM.
        """
    ...
    def _on_config_change(self, change: "ConfigChange") -> None:
        """Push reloaded ``feishu`` authorization fields at the live transport.

        Fails closed on a degraded section: ``allowed_open_ids`` authorises
        nobody when empty, so rebuilding it from a document the loader could not
        parse would lock out every intended sender.
        """
        if self.transport is None or not change.touched("feishu"):
            return
        degraded = change.new.degraded_sections
        if "feishu" in degraded or DEGRADED_WHOLE_CONFIG in degraded:
            logger.warning(
                "Feishu: the reloaded config is degraded; keeping the previous allow-lists "
                "and group gate"
            )
            return
        self.transport.reconfigure(change.new.feishu)
    ...
    def _seed_gen(self, route: tuple[str, str]) -> int:
        ...
        return seed_generation(
            ...
            agent=self._resolve_agent(),
            user_id=comp,
            dm_scope=self._dm_scope(route, 0),
            chat_type=slot,
        )
