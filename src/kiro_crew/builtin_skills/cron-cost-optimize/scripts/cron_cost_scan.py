...
It reads NOTHING from disk. Input is the JSON document ``cron_list`` produces
with ``json=true``, fed on stdin. That is deliberate and it is the whole design:
...
#: A skill named by word, for jobs that spell it out in prose.
