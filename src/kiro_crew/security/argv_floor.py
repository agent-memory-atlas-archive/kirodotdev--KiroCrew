...
def _resolve_host_verdict_into_cache(host: str) -> None:
    ...
    own-address set, marks the host self.  A name that does not resolve is
    not self: a connection cannot reach this machine through it either.
    """
    verdict = False
    try:
...
def _operand_targets_self(operand: str, *, host_position: bool = True) -> bool:
    ...
    # (``127.0.0.$((0+1))`` -> ``127.0.0.1``), so fail closed -- checked
    # for the positional/host slot and for the ``host:path`` colon form.
    if "$((" in normalized and (host_position or ":" in normalized):
        return True
...
def _is_ssh_to_self(text_lower: str) -> bool:
    ...
    for tokens in _self_token_frames(text_lower):
        ...
        rsync_rsh_exported: "str | None" = None
        # The last plain assignment seen in this frame, wherever it appeared:
        # a shell variable persists for the rest of the line even after the
        # simple command it prefixed ends, so a later bare ``export RSYNC_RSH``
        # (the POSIX ``VAR=value; export VAR`` two-step) promotes it into the
        # environment.  Never cleared by separators.
        rsync_rsh_assigned: "str | None" = None
        ...
        for i, token in enumerate(tokens):
            ...
            if verb is None:
                ...
                if assign is not None:
                    ...
                    rsync_rsh_pending = value
                    rsync_rsh_assigned = value
                    ...
                    if prev_stripped_tok == "export":
                        rsync_rsh_exported = value
                else:
                    if (
                        prev_stripped_tok == "export"
                        and stripped_tok.rstrip(";&") == "rsync_rsh"
                        and rsync_rsh_assigned is not None
                    ):
                        # Bare ``export RSYNC_RSH`` after an earlier plain
                        # assignment: the stored value enters the environment.
                        # The walk reads lowered text, and a glued separator
                        # (``export RSYNC_RSH;``) rides on the name token.
                        rsync_rsh_exported = rsync_rsh_assigned
            ...
            if verb == "rsync":
                rsync_rsh = (
                    rsync_rsh_pending if rsync_rsh_pending is not None else rsync_rsh_exported
                )
                ...
                if rsync_rsh is not None:
                    if _is_ssh_to_self(rsync_rsh):
                        return True
                elif rsync_rsh_carried_self:
                    return True
        ...
        carried_now = rsync_rsh_pending if rsync_rsh_pending is not None else rsync_rsh_exported
