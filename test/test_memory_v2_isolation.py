...
class TestConsolidatorStoreResolution:
    ...
    def test_session_store_name_keeps_v1_absence_and_valid_private_identity(
        self, silos, meta, expected
    ) -> None:
        assert _session_store_name(meta) == expected

    @pytest.mark.parametrize("value", [None, 7, [CODING], "   ", " coding ", "no-such-store"])
    def test_invalid_recorded_identity_stops_consolidation(self, silos, value) -> None:
        with pytest.raises(UnknownMemoryStore):
            _session_store_name({"memory_store": value})
        assert silos.global_vectors.get_all_semantic() == []
...
class TestTheSessionsRecordedBindingIsWhatASurfaceReads:
    ...
    protected binding is the authoritative crew identity in scope. Matching
    transcript metadata is what ``history_consolidation._session_store_name``
    resolves on the write side, so the pair makes one conversation's reads and
    writes name one silo.
    """
    ...
    def test_a_recorded_silo_is_the_store_the_surface_resolves(self, silos, tmp_path) -> None:
        ...
        # The write side gets the same answer from matching transcript metadata.
        assert _session_store_name(log.get_metadata(_CHANNEL_SESSION_KEY)) == CODING

    @pytest.mark.parametrize("value", [None, 17, "   ", " coding ", "no-such-store"])
