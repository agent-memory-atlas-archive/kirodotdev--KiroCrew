...
async def api_memory_episodic_correct(request: web.Request) -> web.Response:
    """PATCH /api/memory/episodic: owner correction in one private store."""
    denial = await require_owner_dashboard_request(request, "memory.episodic.correct")
    ...
    name, error = await _admin_store(request, "memory.episodic.correct", body)
    if error is not None:
        return error
    tier, error = await _private_tier(request, name)
    if error is not None:
        return error
    mem_id = body.get("id")
    if not isinstance(mem_id, str) or not mem_id or len(mem_id) > 128:
        return _error("A valid episode id is required.", "invalid_episode_correction")
    try:
        outcome = await asyncio.to_thread(
            tier.correct_episodic,
            mem_id,
            body.get("text"),
            tags=body.get("tags"),
        ...
        return _error(str(exc), "invalid_episode_correction")
    except (OSError, sqlite3.Error):
        return _store_unavailable(name)
    if outcome == "missing":
        return _error(
            "This episode is no longer active. Refresh the list.", "episode_not_found", 404
        )
    if outcome == "conflict":
        return _error("Another active episode already has this text.", "episode_conflict", 409)
    await _audit(request, "memory.episodic.correct", outcome, name)
    return web.json_response(
        {"ok": True, "store": name, "id": mem_id, "changed": outcome == "corrected"}
    )
