...
class LaunchResult:
    ...
    def as_dict(self) -> dict[str, Any]:
        ...
        return {"ok": self.ok, "session": self.session, "error": self.error}
...
def _list_argv(cli: str) -> list[str]:
    return [cli, "--json", "list"]


def _goto_argv(cli: str, session: str, url: str) -> list[str]:
    return [cli, f"-s={session}", "goto", url]


def _open_argv(cli: str, session: str, url: str) -> list[str]:
    return [cli, f"-s={session}", "open", url]


def _close_argv(cli: str, session: str) -> list[str]:
    return [cli, f"-s={session}", "close"]
...
def _session_is_open(cli: str, session: str, env: dict[str, str]) -> bool | None:
    ...
    """
    rc, out, _err = _run_cli(_list_argv(cli), env, _LIST_TIMEOUT_S)
    if rc != 0:
        return None
    try:
        data = json.loads(out)
...
def _reveal(session: str, env: dict[str, str]) -> None:
...
def open_url(url: str, session_key: str) -> LaunchResult:
    ...
    with _session_lock(session):
        if _session_is_open(cli, session, env) is False:
            rc, out, err = _run_cli(_open_argv(cli, session, url), env, _OPEN_TIMEOUT_S)
            if rc == _TIMEOUT_RC:
                # The CLI was killed waiting, but the detached daemon it started
                # may well be up by now; record it so shutdown reaps it instead
                # of leaving it for a retry that may never come.
                with _lock:
                    _opened.add(session)
        else:
            rc, out, err = _run_cli(_goto_argv(cli, session, url), env, _GOTO_TIMEOUT_S)
        if rc != 0:
        ...
        with _lock:
            _opened.add(session)
    _reveal(session, env)
    return LaunchResult(True, session, None)


def _own_open_sessions(cli: str, env: dict[str, str]) -> list[str] | None:
...
def reclaim_stranded() -> int:
    ...
    if cli is None:
        return 0
    stranded = _own_open_sessions(cli, _launch_env(""))
    if not stranded:
        return 0
    closed = 0
    for session in stranded:
        ...
        with _session_lock(session):
            ...
            rc, _out, err = _run_cli(
                _close_argv(cli, session), _launch_env(session), _CLOSE_TIMEOUT_S
            )
        if rc == 0:
            closed += 1
        else:
...
def close_all() -> None:
    ...
    for session in sessions:
        rc, _out, err = _run_cli(_close_argv(cli, session), _launch_env(session), _CLOSE_TIMEOUT_S)
        if rc != 0:
