...
class TestLoopbackBrowserPortIsHubOwned:
    ...
    def test_the_pane_port_is_not_the_destination_port(self, tmp_path, monkeypatch):
        ...
        params = stm._TransportParams(method=CONNECTION_METHOD_LOOPBACK)
        monkeypatch.setattr(stm, "_is_port_free", lambda port: True)
        got = asyncio.run(mgr._acquire_local_port(inst, params))
        assert got != inst.remote_port
    ...
    def test_the_carve_out_does_not_excuse_a_changed_identity(self, monkeypatch):
        """It excuses only a MISSING answer. An answer naming another pid on our
        own port is still a change, and still condemns."""
        from kiro_crew.instances import ssh_tunnel_manager as stm
...
class TestLoopbackTransport:
    """The loopback transport: a fixed destination and the config gate."""
    ...
    def test_connect_is_refused_while_the_transport_is_opted_out(self, tmp_path, monkeypatch):
        ...
        # The resolved params carry no address of their own: the destination is
        # the fixed constant, and nothing on the record can move it.
