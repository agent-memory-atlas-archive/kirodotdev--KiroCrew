...
async def api_agent_publish(request: web.Request) -> web.Response:
    ...
    async with _get_config_lock():
        ...
        # new name — but the publish did not reach its end state, so say so.
        try:
            ...
            return web.json_response(
                {
                    "error": f"'{new_name}' was saved and the crew is bound to it, but it "
                    "could not be marked shared; it remains this crew's private copy.",
