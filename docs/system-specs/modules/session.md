...
- **Idle cleanup**: expires sessions after `session.timeout_secs` (default
  ...
  **re-read every tick**, not frozen at loop start: `_adopt_idle_policy()` runs
  ...
  sentinel, and the non-negative-int coercion of the RSS ceiling. The derived
  sleep interval is recomputed per iteration from the adopted timeout, so a
  shortened timeout also tightens the sweep cadence rather than waiting out the
  old interval. A transition is logged ONCE, not per tick:
...
(`live.subscribe("session", "agent", "watchdog", "agents",
callback=self._on_config_change, name="SessionManager")`, kept on
