...
describe('MembersPage side panel (Crew summary tab) and edit jump', () => {
  it('shows the read-only config summary and the usable V1 migration choice', async () => {
    ...
    expect(drawer).toHaveTextContent('Memory V1. This member can keep working with its current memory.')
    expect(drawer).toHaveTextContent('A new private Memory V2 starts empty')
    expect(drawer).toHaveTextContent('Memory V1 stays unchanged.')
  })
  ...
  it.each([
    ['a V2 store owned by another member', { memory_store: 'beta-own', memory_version: 2, memory_owner: 'beta' }],
    ['an undeclared named store', { memory_store: 'missing-store' }],
  ])('shows %s as unavailable without offering a V1 downgrade', async (_case, memory) => {
    await renderPage([row({ bound: true, slot_key: 'member-oncall', ...memory })])
    fireEvent.click(await screen.findByText('oncall'))
    const drawer = await screen.findByTestId('member-crew-summary')
    expect(drawer).toHaveTextContent(/configured memory store is unavailable/i)
    expect(drawer).not.toHaveTextContent(/can keep working with its current memory/i)
    expect(drawer).not.toHaveTextContent(/only this member can use it/i)
    expect(screen.getByRole('button', { name: 'Set up in crew manager' })).toBeVisible()
  })
