...
class TestSpecCheckRunsWhereTheGateIsImportable:
    ...
    def test_the_windows_interpreter_carries_the_exe_suffix(self, tmp_path, monkeypatch):
        """``Scripts/kirocrew.exe`` -> ``Scripts/python.exe``. Only the suffix
    ...
    def test_no_launcher_leaves_no_interpreter_to_reach(self, monkeypatch):
        script = self._script()
    ...
    def test_the_child_s_verdict_is_this_check_s_verdict(self, tmp_path, monkeypatch):
        ...
        assert seen["argv"][1] == os.path.abspath(script.__file__)
        assert seen["argv"][2:] == ["--spec", str(spec)]
