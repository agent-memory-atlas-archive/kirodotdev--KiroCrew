...
def _write(path: Path, doc: dict) -> None:
    ...
    # Force a distinct mtime so the fingerprint moves on coarse filesystems too.
    st = path.stat()
...
class TestRefreshNow:
    ...
    @pytest.mark.asyncio
    async def test_raising_applier_does_not_block_next(self, cfg_file: Path) -> None:
        w = ConfigWatch()
        w.prime(KiroCrewConfig.load())
        order: list[str] = []
        ...
        w.subscribe("agent", callback=_boom, name="boom")
        w.subscribe("agent", callback=lambda c: order.append("ok"), name="ok")
        _write(cfg_file, {"agent": {"model": "model-cc", "log_level": "INFO"}})
    ...
    @pytest.mark.asyncio
    async def test_load_failure_keeps_previous_snapshot(self, cfg_file: Path) -> None:
        w = ConfigWatch()
        w.prime(KiroCrewConfig.load())
        before = w.snapshot()
        with patch.object(ConfigWatch, "_load", side_effect=OSError("disk gone")):
            assert await w.refresh_now() is None
        assert w.snapshot() is before
        assert w.last_error is not None and "disk gone" in w.last_error
    ...
    @pytest.mark.asyncio
    async def test_notify_written_wakes_loop(self, cfg_file: Path) -> None:
        w = ConfigWatch(poll_interval_secs=30.0)
        got = asyncio.Event()
        w.subscribe("agent.model", callback=lambda c: got.set(), name="m")
        await w.start(initial=KiroCrewConfig.load())
        try:
            _write(cfg_file, {"agent": {"model": "model-dd", "log_level": "INFO"}})
            w.notify_written()
...
class TestRequiresRestart:
    def test_marked_fields(self) -> None:
        from kiro_crew.config.schema import requires_restart

        assert requires_restart("agent.jail")
        assert requires_restart("dashboard.tailscale.trust_identity")
        assert requires_restart("tunnel.enabled")
        assert requires_restart("instances.tunnel_base_port")
    ...
    def test_hot_fields_and_ancestor_resolution(self) -> None:
        ...
        # A key nested under a marked leaf inherits the mark.
...
class TestHandlerRestartRequired:
    def test_changed_paths_need_restart_uses_metadata(self) -> None:
        from kiro_crew.dashboard.handlers.core import _changed_paths_need_restart

        assert not _changed_paths_need_restart(["agent.max_subagents", "agent.subagent_max_turns"])
        assert _changed_paths_need_restart(["agent.model", "agent.jail"])
        assert not _changed_paths_need_restart([])

    def test_startup_read_keys_set_is_gone(self) -> None:
        from kiro_crew.dashboard.handlers import core

        assert not hasattr(core, "_STARTUP_READ_AGENT_KEYS")
...
class TestChannelManagerSetters:
    def test_setters_adopt_new_caps(self, tmp_path: Path) -> None:
        from kiro_crew.channel import ChannelManager

        mgr = ChannelManager(max_channels=2, max_agents=3, channels_dir=str(tmp_path))
        mgr.set_max_channels(5)
        mgr.set_max_agents(7)
        assert mgr._max_channels == 5
        assert mgr._max_agents == 7
        mgr.set_max_channels(0)
        assert mgr._max_channels == 1  # never below one
...
class TestServerAppliers:
    ...
    def test_workflow_timeout_applier_pushes_the_new_ceiling(self) -> None:
        class FakeWorkflowService:
            def __init__(self) -> None:
                self.timeout_secs = 600

            def set_timeout_secs(self, secs: int) -> None:
                self.timeout_secs = secs

        svc = FakeWorkflowService()
        subs = self._register(SimpleNamespace(workflow_service=svc, channel_manager=None))
        sub = subs["agent.workflow_run_timeout_secs"]
        assert sub.prefixes == ("agent.workflow_run_timeout_secs",)
        cb = sub.callback()
    ...
    def test_workflow_timeout_applier_ignores_an_untouched_change(self) -> None:
        class FakeWorkflowService:
            def __init__(self) -> None:
                self.timeout_secs = 600

            def set_timeout_secs(self, secs: int) -> None:  # pragma: no cover - must not run
                raise AssertionError("applier fired on an unrelated change")

        svc = FakeWorkflowService()
        subs = self._register(SimpleNamespace(workflow_service=svc, channel_manager=None))
        subs["agent.workflow_run_timeout_secs"].callback()(  # type: ignore[misc]
            ConfigChange(old=None, new=self._cfg(), changed=frozenset({"agent.model"}))
        )
        assert svc.timeout_secs == 600

    def test_workflow_timeout_applier_no_ops_without_a_service(self) -> None:
    ...
    def test_channel_limits_applier_pushes_each_cap_independently(self) -> None:
        class FakeChannelManager:
            def __init__(self) -> None:
                self.channels = 2
                self.agents = 3

            def set_max_channels(self, n: int) -> None:
                self.channels = n

            def set_max_agents(self, n: int) -> None:
                self.agents = n

        mgr = FakeChannelManager()
        subs = self._register(SimpleNamespace(workflow_service=None, channel_manager=mgr))
        sub = subs["channel_limits"]
        assert sub.prefixes == ("agent.max_channels", "agent.max_channel_agents")
        cb = sub.callback()
        assert cb is not None
        ...
        cb(
            ConfigChange(
                old=None,
                new=self._cfg(max_channels=9, max_channel_agents=4),
                changed=frozenset({"agent.max_channels"}),
            )
        )
        assert (mgr.channels, mgr.agents) == (9, 3)  # only the touched cap moved

        cb(
            ConfigChange(
                old=None,
                new=self._cfg(max_channels=9, max_channel_agents=11),
                changed=frozenset({"agent.max_channel_agents"}),
            )
        )
        assert (mgr.channels, mgr.agents) == (9, 11)

    def test_channel_limits_applier_no_ops_without_a_manager(self) -> None:
        subs = self._register(SimpleNamespace(workflow_service=None, channel_manager=None))
        subs["channel_limits"].callback()(  # type: ignore[misc]
            ConfigChange(
                old=None, new=self._cfg(max_channels=9), changed=frozenset({"agent.max_channels"})
            )
        )
    ...
    async def test_a_file_write_reaches_the_workflow_ceiling(self, cfg_file: Path) -> None:
        """End to end: config.json -> ConfigWatch -> WorkflowService ceiling."""
        ...
        subs = self._register(SimpleNamespace(workflow_service=svc, channel_manager=None))
        sub = subs["agent.workflow_run_timeout_secs"]
        w = live.watch()  # _register_config_watch subscribed on the singleton
        w.prime(KiroCrewConfig.load())
        assert sub in list(w.subscriptions())
...
class TestDegradedAndUnreadableConfig:
    ...
    snapshot with nothing dispatched (see ``test_load_failure_keeps_previous_snapshot``).
    """
