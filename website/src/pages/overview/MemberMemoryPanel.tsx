...
export default function MemberMemoryPanel({ store, summary, onDirtyChange }: { store: string; summary: MemoryStoreSummary; onDirtyChange?: (dirty: boolean) => void }) {
  ...
  useEffect(() => setCopyConfirmed(false), [store])
  const refresh = () => {
  ...
  const empty = total === 0
  const copyAction = privateMemory && <Btn primary={empty} className="min-h-11 justify-center" onClick={() => { setCopyConfirmed(false); setCopying(true) }} disabled={!summary.exists}><Copy className="lucide-inline" />{t('memoryV2.copy_action')}</Btn>
  ...
  return <div className="min-w-0 space-y-4">
    <Card className="mb-0 min-w-0 py-4">
      <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        {!empty && <div className="shrink-0">{copyAction}</div>}
      </div>
    </Card>
    {copyConfirmed && <p role="status" className="text-[13px] text-muted">{t('memoryV2.copy_source_unchanged')}</p>}
    ...
    <Tabs value={section} layoutId={`member-memory-sections-${store}`} onValueChange={selectSection}>
      ...
      <TabsContent value="memories" forceMount hidden={section !== 'memories'}><MemoryRecordsEditor store={store} privateMemory={privateMemory} onDirtyChange={setRecordsDirty} onRecovery={() => selectSection('recovery')} emptyActions={<>{copyAction}{privateMemory && <Btn className="min-h-11" onClick={openMember}><ArrowUpRight className="lucide-inline" />{t('memoryV2.open_member')}</Btn>}</>} /></TabsContent>
    ...
    {copying && <SeedMemory store={store} member={summary.owner_member || summary.name} onClose={() => setCopying(false)} onCopied={() => { refresh(); setCopyConfirmed(true) }} />}
  </div>
}
