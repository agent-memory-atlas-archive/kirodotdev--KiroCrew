...
@pytest.mark.parametrize("store", ["v2"], indirect=True)
def test_controlled_update_cases_with_historical_rule_contrast(store, case):
    """Four controlled regression cases, not a held-out language-model benchmark.

    The baseline expression documents the old confidence=1 escalation rule;
    it does not execute an old binary or measure general memory quality.
    """
    before = fact(store)
    ...
    baseline_final = item["value"]  # Actual old confidence=1 -> user_explicit replacement rule.
    assert (baseline_final == expected) == (case == "explicit")
    writer = object.__new__(HistoryConsolidator)
    writer._write_structured_memory(
        {"semantic": [item]},
        "dashboard:alice",
