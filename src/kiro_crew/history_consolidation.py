    ...
    def _save_lessons(
        ...
        *,
        ...
        if vector_store:
            ...
            lesson_source = (
                "consolidation"
                if getattr(vector_store, "algorithm_version", "v1") == "v2"
                else "user_explicit"
            )
            for item in raw:
        ...
        if isinstance(semantic_items, list):
            ...
            for item in semantic_items[:_MAX_SEMANTIC_PER_CONSOLIDATION]:
                ...
                if not math.isfinite(conf) or not 0 <= conf <= 1:
                    skipped += 1
                    continue
                # V1 keeps its existing confidence policy; V2 retains the actual origin.
                item_source = "user_explicit" if not private_policy and conf >= 1.0 else source
                extra = {}
