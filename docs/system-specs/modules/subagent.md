...
(`live.subscribe("agent", "session.pool_size", callback=self._on_config_change,
name="SubagentManager")`, kept on `self._config_sub`; the watcher holds the
bound method weakly, so a discarded manager drops out on its own).

### `reconfigure(cfg, *, max_concurrent=None)` — live config
...
| `agent.approval_mode` | `_global_approval_mode` (parentless spawns) | `str` |
| `agent.completion_keep`, `agent.completion_keep_chars` | `_completion_keep`, `_completion_keep_chars` | via `update_completion_keep` |

The applier (`_on_config_change`) resolves the cap with `asyncio.to_thread`
because auto-sizing reads `/proc/meminfo` / cgroup files, then calls
`reconfigure(cfg, max_concurrent=cap)`; a direct `reconfigure(cfg)` resolves
