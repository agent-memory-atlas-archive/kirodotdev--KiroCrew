...
- **No cgroups, so the ceiling is a Job object — and it IS enforced.**  `supervise_gateway` creates the gateway `CREATE_SUSPENDED`, attaches a Job
...
Boot Canary (Windows)** (`test/test_pod_windows_boot.py`), which registers a
real task on a per-run plane, boots the pod, and asserts `pod down` leaves no
task, home or env file behind. The module is opt-in: it skips unless
