...
def test_scale_probe_uses_supported_search_and_edits_only_matching_records(tmp_path):
    from kiro_crew.eval.bench.memory_edit import measure

    results = measure(tmp_path, rows=11)
    assert [result["version"] for result in results] == ["v1", "v2"]
    assert all(result["email_matches"] == result["edited"] == 3 for result in results)
