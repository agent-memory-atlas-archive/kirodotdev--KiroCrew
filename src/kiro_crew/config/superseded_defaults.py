...
SUPERSEDED_DEFAULTS: tuple[SupersededDefault, ...] = (
    ...
    #
    # THE ONE ENTRY THAT STAYS REPORT-ONLY, and the reason ``auto_adopt`` is
    # per-entry rather than a global switch. A stored ``False`` here is ALSO the
    # documented opt-out for a server that must not share a backend
    # (``test_a_real_false_still_turns_it_off`` in the gateway env suite pins it),
    # so un-materializing it would turn pooling ON for an operator who deliberately
    # turned it off -- a behaviour change, not a budget. Every other entry moves one
    # number or one name on one axis; this one carries two meanings in the same
    # bytes, which is exactly the case no automatic rewrite can decide.
    SupersededDefault(
    ...
    # it must be a conscious act. 90.0 is also the maximum the validator accepts, so
    # "max the slider out" is an ordinary deliberate choice that lands on exactly
    # these bytes -- and the cost of holding it is money, not a broken feature.
    SupersededDefault(
    ...
    #
    # REPORT-ONLY, for the reason a BOOLEAN cannot be adopted safely: the field has
    # two possible values, so EVERY user who deliberately turns streaming off stores
    # exactly the old default. The collision is not unlikely, it is certain, and the
    # dashboard has a control that produces it -- ``test_put_persists_streaming``
    # PUTs ``streaming: false`` and pins that it survives a fresh load, which an
    # adoption would break. A number chosen out of a range is the opposite case: a
    # stored 1800 out of 60..86400 is a coincidence, not the shape of an opt-out.
    SupersededDefault(
    ...
    # seconds for managed services.
    #
    # REPORT-ONLY, and this one is pinned twice:
...
def _update_map(
    ...
    **The write never RESOLVES the leaf.** ``write_config_atomically`` deliberately
    ...
    Raises ``OSError`` (including :class:`AckPathRefused` and, on a contended
    ``wait_for_lock=False`` acquire, ``BlockingIOError``) on any filesystem refusal;
    callers turn that into a controlled CLI error or a deferral rather than a
    traceback.
    """
    path = ack_file_path()
