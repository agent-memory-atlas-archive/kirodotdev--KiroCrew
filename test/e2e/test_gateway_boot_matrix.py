...
def test_shutdown_leaves_no_gateway_child_alive() -> None:
    ...
    Two observations, because neither alone is sufficient. The gateway pid being
    gone says nothing about the MCP servers and agent children it spawned, and
    those are what outlive a ``proc.terminate()`` that signals only the parent.
    A REFUSED connect to the dashboard port proves no process in that tree still
    holds the listener, without needing a process-tree walk or a new dependency.
    ...
    assert _port_is_refused(port), (
        f"port {port} still has a listener after teardown; a child of the " "gateway outlived it"
    )
