...
def _create_store_dir(name: str) -> None:
    ...
    ``ensure_memory_store_dir`` requires the declaration before path creation.
    The re-check proves the config write is visible to that resolver; it never
    substitutes the operator's workspace for an unavailable named identity.
