...
a plain attribute read) or, before the watcher is armed, the fingerprint-cached
`KiroCrewConfig.load()`, and re-applies the clamp above to the parallel cap and
...
without a restart. Three rules keep this predictable:
