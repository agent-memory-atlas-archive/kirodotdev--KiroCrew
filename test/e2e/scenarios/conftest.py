...
# The kernel's sockaddr_un.sun_path cap, minus room for the trailing NUL.
_AF_UNIX_MAX = 100
...
def _plane_root(name: str) -> Path:
    """A plane root SHORT enough that the pod's unix socket path fits AF_UNIX.

    Not `tmp_path_factory`. A pod's gateway binds its private dashboard socket at
    ``<pod home>/dashboard-<port>.sock``, and AF_UNIX caps a path at 108 bytes.
    Under a pytest temp root the full path blew that cap, the gateway logged
    ...
    # TMPDIR first, because that is the session-owned scratch the runtime
    # reclaims; `/tmp` only as the fallback for the common case where TMPDIR is
    # itself already too deep to host a socket.
    tried: list[tuple[Path, int]] = []
    for base in (Path(tempfile.gettempdir()), Path("/tmp"), Path.home()):
        root = base / leaf
        width = _worst(root)
...
def pod():
    ...
    if probe.returncode != 0:
        # The plane root already exists and lives outside pytest's temp tree, so
        # an ordinary precondition skip must not leave it behind either.
    ...
    try:
        ...
        # itself: it lives outside pytest's temp tree (see _plane_root), so
