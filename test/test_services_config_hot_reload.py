...
def _fresh_watcher():
    """Every test gets an unstarted process watcher with an empty registry."""
    live.reset_for_tests()
    yield
    live.reset_for_tests()


def _change(new, *paths: str, old=None) -> ConfigChange:
    """The shape a subscriber receives from one reload."""
    return ConfigChange(old=old, new=new, changed=frozenset(paths))
...
class TestVectorMemoryStore:
    ...
    def test_reconfigure_adopts_every_retrieval_tunable(self, tmp_path: Path) -> None:
        ...
        store._on_config_change(_change(cfg, "memory.semantic_confidence_threshold"))
    ...
    def test_semantic_keys_are_appended_to_the_builtin_prefixes(self, tmp_path: Path) -> None:
        ...
        store = _memory_store(tmp_path)
        store.reconfigure(_memory_cfg(semantic_keys=["extra/"]).memory)
        ...
        store.reconfigure(_memory_cfg(semantic_keys=[]).memory)
    ...
    def test_decay_rates_are_re_sanitized_not_copied(self, tmp_path: Path) -> None:
        ...
        store = _memory_store(tmp_path)
        store.reconfigure(
            _memory_cfg(decay_rates={"default": 0.07, "work": 0.5, "junk": "not-a-rate"}).memory
        )
    ...
    def test_the_resident_episodic_scoring_set_is_invalidated(self, tmp_path: Path) -> None:
        ...
        store.reconfigure(_memory_cfg(decay_rates={"default": 0.09}).memory)
...
class TestEpisodicMaxCountIsConsumed:
    ...
    def test_a_reload_pushes_the_count_onto_a_store_that_missed_it(self, tmp_path: Path) -> None:
        ...
        store._on_config_change(_change(_memory_cfg(episodic_max=900), "memory.episodic_max_count"))
...
class TestHistoryConsolidator:
    ...
    def test_reconfigure_adopts_all_twelve_settings(self) -> None:
        c = _consolidator()
        c._on_config_change(_change(_skills_cfg(), "skills.max_auto_skills"))
    ...
    @pytest.mark.asyncio
    async def test_a_raising_applier_is_contained_by_the_dispatcher(self) -> None:
        c = _consolidator()
        c.reconfigure(_skills_cfg(max_auto_skills=42))
        broken = _skills_cfg()
        broken.skills.max_auto_skills = "lots"  # type: ignore[assignment]
        await _dispatch(broken, "skills.max_auto_skills")
        assert c._max_auto_skills == 42
...
class TestCronHistoryStore:
    ...
    def test_reconfigure_adopts_the_caps(self, tmp_path: Path) -> None:
        store = _cron_history_store(tmp_path)
        store._on_config_change(_change(_cron_history_cfg(), "cron_history.cron_summary_cap"))
    ...
    def test_the_trace_cap_is_re_multiplied_to_bytes(self, tmp_path: Path) -> None:
        ...
        store = _cron_history_store(tmp_path)
        store.reconfigure(_cron_history_cfg(trace_kb=64).cron_history)
...
class TestSkillsLoader:
    ...
    def test_reconfigure_adopts_a_newly_added_root(self, tmp_path: Path) -> None:
        loader = _skills_loader(tmp_path)
        added = tmp_path / "added"
        added.mkdir()
        cfg = KiroCrewConfig()
        cfg.skills.extra_paths = [str(added)]
        loader._on_config_change(_change(cfg, "skills.extra_paths"))
...
class TestSshTunnelManagerApplyConfig:
    ...
    def test_apply_config_adopts_every_transport_tunable(self) -> None:
        manager = _tunnel_manager()
        manager._on_config_change(_change(_instances_cfg(), "instances.connect_timeout_secs"))
...
class TestLLMPoolReconfigure:
    ...
    async def test_a_knowledge_write_reaches_the_pool(self) -> None:
        ...
        with patch(
            "kiro_crew.knowledge.llm_pool._read_config",
            return_value={"knowledge": {"pool_idle_ttl_secs": 45}},
        ):
            await _dispatch(KiroCrewConfig(), "knowledge.pool_idle_ttl_secs")
...
class TestResponseSpillThresholdReadAtUse:
    ...
    def test_the_boot_constant_is_kept_for_fixed_number_callers(self) -> None:
        from kiro_crew.mcp_gateway import pool as pool_mod

        assert isinstance(pool_mod.RESPONSE_SPILL_THRESHOLD_BYTES, int)

    def test_the_backend_resolves_through_the_function_not_the_constant(self) -> None:
        """One resolve per response serves both the size test and the spill argument."""
        from kiro_crew.mcp_gateway import backend as backend_mod

        assert callable(backend_mod.response_spill_threshold_bytes)
...
class TestResolveOnceRefreshWindowIsLive:
    ...
    def test_without_a_snapshot_it_falls_back_to_a_load(self) -> None:
        ...
        with patch.object(gateway_mod.KiroCrewConfig, "load", return_value=cfg) as load:
