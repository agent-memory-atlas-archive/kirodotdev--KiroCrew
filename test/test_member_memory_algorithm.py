...
from kiro_crew import memory_stores, memory_v2
...
def test_consolidation_keeps_v1_updates_deletes_and_lesson_origin(stores):
...
def test_v2_lessons_do_not_delete_distinct_rules_on_topic_overlap(stores):
    _, v2 = stores
    first = "Use database migrations for schema changes in production."
    second = "Review database migrations for schema changes before deployment."
    assert v2.write_lesson(first)
    assert v2.write_lesson(second)
...
def test_episode_correction_clears_stale_vector_and_rolls_back_failed_audit(stores):
    v1, store = stores
    mem_id = episode(store, "Postgres listens on port 5432 locally", cosine=0.95)
    assert (
        store.correct_episodic(mem_id, "Postgres listens on port 5432 locally", importance=0.9)
        == "corrected"
    )
    assert (
        store.db.execute("SELECT embedding FROM memory_items WHERE id=?", (mem_id,)).fetchone()[0]
        is not None
    )
    store.db.execute(
    ...
    with pytest.raises(Exception, match="audit unavailable"):
        store.correct_episodic(mem_id, "Postgres listens on port 6432 locally")
    assert "5432" in store.get_episodic_list()[0]["text"]
    store.db.execute("DROP TRIGGER reject_correction")
    assert store.correct_episodic(mem_id, "Postgres listens on port 6432 locally") == "corrected"
    assert (
        store.db.execute("SELECT embedding FROM memory_items WHERE id=?", (mem_id,)).fetchone()[0]
        is None
    )
    ...
    with pytest.raises(ValueError, match="private V2"):
        v1.correct_episodic(mem_id, "Postgres listens on port 6432 locally")
