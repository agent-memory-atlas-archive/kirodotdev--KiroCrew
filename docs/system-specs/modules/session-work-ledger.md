...
The same funnel releases the cron jobs of the deleted session: the single delete runs a locked cron owner sweep before the transcript unlink and again after it (the bulk clear runs one batched pre-sweep for the whole selection), and `_remove_slot_for_history_key()` hands the collected owner keys to `CronService.release_jobs_owned_by()` last — after the turn cancellation, the ledger purge and the autocompact-override sweep — so a dying turn cannot re-stamp its key on a job after the release. The failure modes of the cron store itself (`cron_store_unreadable`, `cron_store_busy`, and on the single delete `cron_ownership_unknown`) refuse the delete with 409 before anything is unlinked, so the ledger is untouched; any other cron error is logged with the by-id recovery command (`kirocrew cron adopt <id> --release`) and never blocks the ledger purge (see [learn-cron-dashboard](learn-cron-dashboard.md)).
