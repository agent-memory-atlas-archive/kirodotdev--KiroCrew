...
function TemplateUpdatedNotice({ job }: { job: CronJob }) {
  ...
  // Key on the value compared, so a NEW template change re-shows the hint.
  const key = update ? `kc-tpl-upd-dismissed:${job.id}:${simpleHash(update.currentPrompt)}` : ''
...
/** Small stable string hash (djb2) for a localStorage key -- not security, just
 *  a compact fingerprint of the compared prompt so the dismissal re-arms when
 *  the template prompt changes again. */
