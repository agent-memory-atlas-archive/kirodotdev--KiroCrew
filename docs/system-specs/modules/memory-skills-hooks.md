...
preserves the V1 source; Copy knowledge transfers only selected records.
...
Copy knowledge explains source preservation inside the dialog before selection,
...
Ranking combines cosine and query coverage (0.7/0.3 when a vector exists;
coverage alone without a vector) and modest importance weighting. Age never
...
the previous text or resurrect a forgotten row. Owner-only
`PATCH /api/memory/episodic` accepts `{store, id, text, tags?, importance?}` for
an active V2 episode. It preserves its id, creation time, session and provenance;
records `user_explicit`, updated time and an atomic before/after audit event;
and clears the stale vector only when text changes. Invalid text/metadata,
another active episode with identical text and an absent/forgotten id refuse
without mutation. An identical retry returns `changed: false`. Global V1 has no
entry to this correction path.
...
names; Global V1 retains its original database backup names and behavior.
...
**V1 restore is staged and recoverable.** `restore_from_backup` uses SQLite's
online backup API to stage a self-contained copy, verifies integrity, and
...
refresh and explicit lesson source. A consolidation confidence of 1.0 keeps its
legacy `user_explicit` classification. V1's audit log remains best effort.
...
3. **Consolidation** (background): extracts corrections not already saved via `learn_add`. V1 retains `write_lesson(source="user_explicit")`; V2 records `source="consolidation"` at confidence 0.9.
...
discarding already loaded history. Revision snapshots and proposal history have
no automatic retention limit in either V1 or V2. Repeated accepted mutations can
grow the database even when V1's live-record capacity is bounded; forgetting a
record does not erase its audit snapshots. Access timestamps and embedding-only
...
Revisions retain before/after evidence and conflict proposals. Metadata helpers
never commit or embed; each writer owns the transaction with its content.
