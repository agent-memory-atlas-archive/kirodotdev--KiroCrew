...
async def _ingest_local_file_task(pipeline, store, path: str, source_id: str) -> None:  # type: ignore[no-untyped-def]
    ...
    Owns the row's whole sync lifecycle: it claims 'syncing' atomically, returns
    without touching anything if another sync already holds the claim, and then
    stamps 'synced', 'pending' when the import budget defers the ingest, or
    'error'. Holding both ends here is what stops a cancelled request from
    leaving a row 'syncing' with no work behind it.
    """
    try:
        if not await asyncio.to_thread(_claim_sync, store, source_id):
            return
