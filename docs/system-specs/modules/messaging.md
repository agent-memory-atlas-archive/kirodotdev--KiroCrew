...
`live.subscribe("<channel>", "messaging", callback=self._on_config_change,
name="<Channel>Dispatcher")` — and keeps the returned `Subscription` on `self`,
because the watcher holds a bound method WEAKLY so a dispatcher a test builds and
drops does not pin itself into the registry. The handler reads
`change.touched(...)` / `change.under(prefix)` to decide what it owns.
...
admit a room nobody approved. An empty list is a real value on the channels whose
...
audited where it is made. A transport that is not attached yet (`transport` is
...
badge, and starts the transport again. Slack is not in that loop — its socket is
