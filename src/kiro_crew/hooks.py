...
class ToolHookResult:
    ...
    #: False when it is a GRANT: an ``auto_approve_tools`` glob or the
    #: app-own-server rule vouches for who is calling and says nothing about
    #: the call's effect. ``ToolApprovalPolicy.READ_ONLY`` honours only the
    #: former; a surface with an interactive approver treats both alike. The
    #: action alone cannot say which branch produced it, and a result built
    #: outside the factory stays unproven (False) — fail-closed.
    ...
    @staticmethod
    def auto_approve(*, read_only: bool = False) -> ToolHookResult:
        """Auto-approve; ``read_only=True`` marks a classifier verdict, never a grant."""
        ToolHookResult._count(TOOL_AUTO_APPROVE, False)
        return ToolHookResult(action=TOOL_AUTO_APPROVE, read_only=read_only)
...
class HookManager:
    ...
    def on_tool_call(
        ...
        *,
        ...
        ``mcp_server_name`` (``_HOST_READ_ONLY_BUILTIN_TOOLS``). The
        agent-influenced inputs — the ACP ``tool_kind`` and the title — may
        NARROW (a non-read kind refuses) but never prove, so a mutating tool
        ...
        if is_shell:
            ...
            if classifier_only:
                ...
                # (``mcp_tool_name``) with no MCP server behind it. The two
                ...
                if _is_host_read_only_builtin(mcp_tool_name, mcp_server_name):
                    return ToolHookResult.auto_approve(read_only=True)
                return ToolHookResult.allow()
...
# or ``commands`` — a test pins that, so a write-capable built-in (``code``,
...
def _is_host_read_only_builtin(mcp_tool_name: str, mcp_server_name: str) -> bool:
    """True when the host-trusted identity names a known read-only BUILT-IN.

    ``mcp_tool_name`` is ``_meta.kiro.toolName``, which kiro-cli sets for
    built-ins too; ``mcp_server_name`` is set only for MCP-served calls. An MCP
    server may expose a tool that happens to be called ``fs_read``, so a
    non-empty server name fails closed: only a built-in — no server behind it —
    can match. An empty tool name (a backend that omits ``_meta.kiro``, an
