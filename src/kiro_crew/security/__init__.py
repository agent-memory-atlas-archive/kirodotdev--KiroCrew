        ...
        while True:
            ...
            for values in rows:
                ...
                fresh = [
                    ...
                    if (record_id, hashlib.sha256(value.encode()).digest()) not in reported
                ]
                if not fresh:
                    continue
                reported.update(
                    (record_id, hashlib.sha256(value.encode()).digest()) for value in matches
                )
...
def _scan_memory_store(store: VectorMemoryStore, store_name: str, findings: list[dict]) -> None:
    ...
    for entry in store.get_all_semantic():
        ...
        if matches:
            reported.update(
                (f"key:{entry['key']}", hashlib.sha256(value.encode()).digest())
                for value in matches
            )
            findings.append(
                {
    ...
    for entry in store.get_episodic_list(limit=_MEMORY_AUDIT_EPISODIC_LIMIT):
        ...
        if matches:
            reported.update(
                (entry["id"], hashlib.sha256(value.encode()).digest()) for value in matches
            )
            findings.append(
                {
                    "type": "episodic",
