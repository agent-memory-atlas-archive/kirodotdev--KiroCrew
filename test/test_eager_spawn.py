...
class TestEagerSpawn:
    ...
    @pytest.mark.parametrize("restored_store", ["", "member-alice"])
    async def test_private_v2_leaves_provider_allocation_to_first_turn(
        self, allow_resume, restored_store
    ):
        """Fresh and resume-prefetch signals must not pre-register an ordinary
        provider before the real turn pins its protected V2 store."""
        slot = _ChatSlot("t1")
        slot.agent = "alice"
        slot.memory_store = restored_store
        state = _mock_state(slot)
        cfg = _cfg_with_store("member-alice", version=2, owner="alice")
        with (
            ...
            patch.object(
                ...
                return_value=_bindings(
                    agent="alice-agent", alias="alice", memory_store="member-alice"
                ),
            ),
        ):
    ...
    @pytest.mark.asyncio
    async def test_empty_slot_inheriting_private_default_still_skips(self):
        """An empty slot is the configured default member, not proof that the
        conversation uses Global V1; classification follows resolved bindings."""
        slot = _ChatSlot("t1")
        state = _mock_state(slot)
        cfg = _cfg_with_store("member-alice", version=2, owner="alice")
        cfg.agents["alice"] = KiroCrewAgentConfig(memory_store="member-alice")
        cfg.default_agent = "alice"
        bindings = _bindings(agent="alice-agent", alias="alice", memory_store="member-alice")
        with (
            patch.object(chat_runner.KiroCrewConfig, "load", return_value=cfg),
            patch.object(chat_runner, "resolve_agent_bindings", return_value=bindings) as resolve,
        ):
            await _eager_spawn(state, slot)
        resolve.assert_called_once_with(cfg, None)
        state.sessions.get_or_create.assert_not_awaited()

    @pytest.mark.asyncio
    async def test_restored_store_mismatch_never_warms_global_provider(self):
        """A stale or unresolved member alias cannot erase persisted store
        identity by speculatively falling back to the Global V1 provider."""
        slot = _ChatSlot("t1")
        slot.agent = "missing-member"
        slot.memory_store = "member-alice"
        state = _mock_state(slot)
        cfg = _cfg_with_store("member-alice", version=2, owner="alice")
        with (
            patch.object(chat_runner.KiroCrewConfig, "load", return_value=cfg),
    ...
    @pytest.mark.asyncio
    async def test_unresolved_member_never_warms_resolver_global_fallback(self):
        """Without persisted metadata, an unavailable explicit member still
        belongs to the first turn's fail-loud path, not a Global provider."""
        slot = _ChatSlot("t1")
        slot.agent = "missing-member"
        state = _mock_state(slot)
        bindings = _bindings()
