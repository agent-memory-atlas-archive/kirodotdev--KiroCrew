...
it('shows the recovery failure and concrete copy names after a refresh', async () => {
  ...
  expect(screen.queryByText(/Restore is ready/)).toBeNull()
...
it('keeps failed activation distinct from ready staging after cancellation', async () => {
  ...
  expect(await screen.findByText(/Staged restore cancelled/)).toHaveTextContent('restart the Kiro Crew gateway')
...
it.each([false, true])('stages, remounts and cancels recovery with privateMemory=%s', async privateMemory => {
  ...
  expect(screen.getByText(/Before it is activated, the current memory is retained as a recovery copy beside the store/)).toBeVisible()
  fireEvent.click(screen.getByRole('button', { name: 'Confirm restore' }))
  expect(await screen.findByText('Restore is ready. Restart Kiro Crew (the gateway) to finish restoring this backup. The current memory store stays active until then.')).toBeVisible()
  ...
  expect(screen.getByText('To restore a different backup, cancel the staged restore first.')).toBeVisible()
