...
class VectorMemoryStore:
    ...
    def correct_episodic(
        ...
        *,
        ...
        """Correct an active V2 episode atomically, preserving its identity and origin.

        Returns corrected/unchanged/missing/conflict. A text correction clears
        its stale vector; lexical recall works immediately while backfill waits.
        """
        if self.algorithm_version != "v2":
            raise ValueError("Episode correction requires private V2 memory")
        if (
            not isinstance(text, str)
            or not _EPISODIC_TEXT_MIN <= len(text.strip()) <= _EPISODIC_TEXT_MAX
        ):
            raise ValueError("Episode text must contain 10–2000 characters")
        text = text.strip()
        if _contains_injection(text):
            raise ValueError("Episode text contains unsupported instruction markers")
        if tags is not None and (
            not isinstance(tags, list)
            or len(tags) > 10
            or any(not isinstance(tag, str) or len(tag.strip()) > 50 for tag in tags)
        ):
            raise ValueError("Tags must contain at most 10 strings of at most 50 characters")
        ...
        with self._db_lock:
            row = self.db.execute(
                f"SELECT * FROM {self._epi_rel} WHERE id = ? AND is_deleted = 0{self._epi_guard}",
                (mem_id,),
            ).fetchone()
            if row is None:
                return "missing"
            clean_tags = (
                json.dumps([tag.strip().lower() for tag in tags if tag.strip()])
                if tags is not None
                else row["tags"]
            )
            weight = importance if importance is not None else row["importance"]
            if text == row["text"] and clean_tags == row["tags"] and weight == row["importance"]:
                return "unchanged"
            if self.db.execute(
                f"SELECT 1 FROM {self._epi_rel} WHERE text = ? AND id != ? AND is_deleted = 0{self._epi_guard}",
                (text, mem_id),
            ).fetchone():
                return "conflict"
            now = _now_iso()
            before = json.dumps(
                {key: row[key] for key in ("text", "tags", "importance")}, ensure_ascii=False
            )
            after = json.dumps(
                {"text": text, "tags": clean_tags, "importance": weight}, ensure_ascii=False
            )
            try:
                self.db.execute(
                    f"UPDATE {self._epi_rel} SET text = ?, tags = ?, importance = ?, "
                    f"source = 'user_explicit', updated_at = ?, embedding = ? WHERE id = ?{self._epi_guard}",
                    (
                        text,
                        clean_tags,
                        weight,
                        now,
                        row["embedding"] if text == row["text"] else None,
                        mem_id,
                    ),
                )
                self.db.execute(
                    "INSERT INTO memory_events (event_type, memory_type, memory_key, "
                    "old_value, new_value, source, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
                    ("correct", "episodic", mem_id, before, after, "user_explicit", now),
                )
                self._record_mutation(
                    "episode", mem_id, dict(row), "user_explicit", operation="correct"
                )
                self.db.commit()
            except Exception:
                self.db.rollback()
