...
def test_every_level_is_pinned_while_the_tree_is_built(tmp_path, windows_os_home, monkeypatch):
    """The pin is the guarantee, so the call is pinned by argv and by count.

    A handle held on each level blocks a rename or a delete of it and of
    everything above it, which is what stops a component being swapped after it
    is checked. Four levels are pinned: the home plus .aws, sso and cache.
    """
    pinned: list[str] = []
    ...
    os_home = tmp_path / "pod" / "os-home"
    rt._seed_pod_os_home(os_home)
    assert pinned == ["os-home", ".aws", "sso", "cache"]
