...
def test_long_whitespace_prefix_preserves_assignment_expansion():
    whitespace = "\t" * 20_000
    assert list(mcp_cron._iter_local_assignments(whitespace)) == []
    command = whitespace + "A=.s B=sh; cat ~/$A$B/id_rsa"
...
def test_long_non_assignment_words_do_not_hide_later_assignments():
    prefix = "A" * 20_000 + " " + "9" * 20_000 + "=ignored"
    assert list(mcp_cron._iter_local_assignments(prefix)) == []
    command = prefix + "; A=.s B=sh; cat ~/$A$B/id_rsa"
    assert list(mcp_cron._iter_local_assignments(command)) == [("A", ".s"), ("B", "sh")]
    assert _substitute_local_assignments(command).endswith("cat ~/.ssh/id_rsa")
...
def test_script_leaf_swap_never_reads_the_target(monkeypatch, tmp_path, without_nofollow):
    ...
    def no_content_read(*args, **kwargs):
        raise AssertionError("a substituted leaf reached the script content reader")

    monkeypatch.setattr(os, "open", swap_then_open)
    monkeypatch.setattr(os, "fdopen", no_content_read)
...
def test_script_parent_swap_before_metadata_never_reads_the_target(monkeypatch, tmp_path):
    ...
    def no_content_read(*args, **kwargs):
        raise AssertionError("a substituted parent reached the script content reader")
...
def test_script_unknown_descriptor_path_is_refused_before_read(monkeypatch, tmp_path):
    ...
    def no_content_read(*args, **kwargs):
        raise AssertionError("an unverified descriptor reached the script content reader")
