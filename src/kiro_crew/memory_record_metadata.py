...
def sync_record(
    ...
    *,
    ...
    resolves proposals with ``force_revision=True``. Full prior provenance is
    retained in the immutable revision even when the current source changes.
    """
    ...
    db.execute(
        f"INSERT INTO memory_record_meta ({', '.join(columns)}) VALUES ({', '.join('?' for _ in columns)}) "
        "ON CONFLICT(record_id) DO UPDATE SET "
        + ", ".join(f"{key}=excluded.{key}" for key in columns[1:]),
        values,
    )
    _append_revision(
        db,
